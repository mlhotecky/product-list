  ...
  fetch(`${API_PREFIX}/`)
    .then(async (response) => {
      ...
      if (!response.ok) {
        ...
        dispatch(
          getProductStatus(GET_PRODUCTS, REJECTED, `${ERROR} ${error || ""}`)
        );
        return;
      }
    ...
    .catch((error) => {
      dispatch(
        getProductStatus(GET_PRODUCTS, REJECTED, error?.toString() || "")
      );
    });
};
...
export const updateProduct = (id, formValues) => (dispatch) => {
  ...
  fetch(`${API_PREFIX}/${id}`, options)
    ...
    .catch((error) => {
      dispatch(
        getProductStatus(UPDATE_PRODUCT, REJECTED, error?.toString() || "")
      );
    });
};
